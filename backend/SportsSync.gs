...
function syncSports(){
  ...
  var header = ["name","date","endDate","time","location","description","tags","status","section"];
  var rows = [header];
...
function gameRow_(g, p, section){
  ...
  var name = (sport + (g.level ? (" " + g.level) : "") + " " + homeAway +
  ...
  var tags = [sport, g.level, homeAway, (g.league ? "league" : "")].filter(String).join(";");
  return [ name, g.date || "", g.date || "", g.time || "", g.location || "",
           descBits.join(" — "), tags, g.status || "", section ];
}
